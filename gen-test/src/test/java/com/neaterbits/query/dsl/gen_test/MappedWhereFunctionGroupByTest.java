...
public class MappedWhereFunctionGroupByTest extends GEN_BaseTestCase {
    ...
    public void testMappedSingleNamed() {
		verifyIsCompilable(
				...
				".where().lower(Farm::getName).startsWith(\"farm\")");
    ...
    public void testMappedSingleAlias() {
		verifyIsCompilable(
				...
				".where().lower(f::getName).startsWith(\"farm\")");
    ...
    public void testMappedMultiNamed() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			.map    ( Farm::getFarmId   ).to(FarmInfo::setFarmId)
    			.where().upper( Farm::getSubFarmId).isEqualTo("SUB2")
    			.groupBy( Farm::getFarmId )
    			.build(); 
    ...
    public void testMappedMultiAlias() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			.map    ( f::getFarmId   ).to(FarmInfo::setFarmId)
    			.where().upper( f::getSubFarmId).isEqualTo("SUB2")
    			.groupBy( f::getFarmId )
    			.build(); 
