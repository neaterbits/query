...
public class MappedWhereAndFunctionTest extends GEN_BaseTestCase {
    ...
    public void testMappedSingleNamed() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			...
    			.where(Farm::getSubFarmId).contains("sub")
    			  .and(Farm::getFarmId).endsWith("2")
    			  .and().lower(Farm::getName).startsWith("farm")
    			  .and().upper(Farm::getName).endsWith("ARM1")
    			.build(); 
    ...
    public void testMappedSingleAlias() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			...
    			.where(f::getSubFarmId).contains("sub")
    			  .and(f::getFarmId).endsWith("2")
    			  .and().lower(f::getName).startsWith("farm")
    			  .and().upper(f::getName).endsWith("ARM1")
    			.build(); 
    ...
    public void testMappedMultiNamed() {
    	...
    	final Farm other3 = new Farm("Other3", "other3", "sub2");
    	final Farm other4 = new Farm("Other4", "other1", null);
    	final Farm other5 = new Farm("Other5", "otherFarm3", "sub2");

    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			.map(Farm::getName).to(FarmInfo::setName)
    			.where(Farm::getFarmId).endsWith("3")
    			  .and(Farm::getSubFarmId).endsWith("2")
      			  .and().lower(Farm::getName).startsWith("other")
    			  .and().upper(Farm::getName).startsWith("OTHER")
    			.build(); 
...
//					new FarmInfo("Farm4"),
    ...
    public void testMappedMultiAlias() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			.map(f::getName).to(FarmInfo::setName)
    			.where(f::getFarmId).endsWith("3")
    			  .and(f::getSubFarmId).endsWith("2")
    			  .and().lower(f::getName).startsWith("other")
    			  .and().upper(f::getName).startsWith("OTHER")
    			.build(); 
