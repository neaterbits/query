...
public class MappedWhereAndFunctionGroupByHavingTest extends GEN_BaseTestCase {
    ...
    public void testMappedMultiNamed() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			...
    			.and().upper(Farm::getName)	 .startsWith("FARM")
    ...
    public void testMappedMultiAlias() {
    	...
    	MultiBuilt<FarmInfo> query = select.list(FarmInfo.class)
    			...
    			.and().upper(f::getName)  .startsWith("FARM")
    			.groupBy( f::getFarmId )
