...
public class AggregateWhereAndTest extends GEN_BaseTestCase {
    ...
    public void testAggregateSingleNamed() {
    	...
    	verifyIsNotCompilable(LandPlot.class, "l",
    		    ...
    		    ".and(l::getHectares).isGreaterThan(new BigDecimal(\"30.5\"))");

    	verifyIsCompilable(LandPlot.class, "l",
    			"sum(LandPlot::getHectares)" +
    			".where(LandPlot::getHectares).isLessThan(new BigDecimal(\"99.5\"))" +
    		    ".and(LandPlot::getHectares).isGreaterThan(new BigDecimal(\"30.5\"))");
    	    	
    	final SingleBuilt<BigDecimal> sumQuery
    							= select.sum(LandPlot::getHectares)
    									.where(LandPlot::getHectares).isLessThan(new BigDecimal("101.5"))
    									.and(LandPlot::getHectares).isGreaterThan(new BigDecimal("40.0"))
    									.build(); 
    	
    	store(land1, land2, land3, land4)
    	.checkAggregate(sumQuery, new BigDecimal("141.0"));
    ...
    public void testAggregateSingleAlias() {
    	...
    	final SingleBuilt<BigDecimal> sumQuery
    							= select.sum(l::getHectares)
    									.where(l::getHectares).isLessThan(new BigDecimal("101.5"))
    									.and(l::getHectares).isGreaterThan(new BigDecimal("40.0"))
    									.build(); 
